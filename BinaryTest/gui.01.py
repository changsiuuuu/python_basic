# BinaryTest/gui01.py

# tkinter import ui Toolkit을 사용할 수 있는 interface 객체
import tkinter as tk


def clicked():
    try :
        print("버튼을 클릭했네요")
        # Entry(입력창)에 입력한 문자열 읽어오기
        name=name_entry.get()
        if  int(name) < 0 or int(name)> 255 :
            label2.config(text="0~255 숫자를 입력하세요")
            return
        # text label 수정하기
        label2.config(text=f"2진수로 변환하면 :{int(name):08b}")
    except ValueError :
        label2.config(text="숫자를 입력하세요")


if __name__ == "__main__" :
    # root 창 생성
    root = tk.Tk()
    # 제목 설정
    root.title("나의 App")
    # 창의 크기
    root.geometry("400x200")

    # 레이블
    label = tk.Label(root, text = "안녕하세요 python GUI입니다")
    #pady = y축 패딩
    label.pack(pady=20)

    # 입력창 만들기
    name_entry = tk.Entry(root, font=("Arial", 12))
    name_entry.pack(pady=5)
    name_entry.focus() # 포커스 주기

    # 버튼
    btn = tk.Button(root, text="전송", command=clicked, width=10, bg="lightgrey")
    btn.pack(pady=15)

    label2 = tk.Label(root, text = "결과는")
    label2.pack(pady=20)

    # 창이 닫힐때까지 무한 대기
    root.mainloop()